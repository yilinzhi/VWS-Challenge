...
def sampling_points(mask, N, k=3, beta=0.75, training=True):
    ...
    B, _, H, W = mask.shape
    ###
    # mask, _ = mask.sort(1, descending=True)# 数值从大到小排列
    ###
    if not training:
        H_step, W_step = 1 / H, 1 / W
        N = min(H * W, N)
        uncertainty_map = -1*torch.abs(0.5-over_generation_map[:,0])
...
if __name__=='__main__':
    x = torch.ones(1, 1, 5, 5, dtype=torch.float, requires_grad=False)
    print(x)
    out = sampling_points(x, 4)
    print(out.shape)
    print(out)