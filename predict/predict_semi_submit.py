...
# @Author  : Yi
# @FileName: predict_semi_submit.py
...
def get_args():
    ...
    parser.add_argument('--model_path_ICAR',
                        default='/data/yilinzhi/Segmentation/VMS/checkpoints/semi_train_F/train/ICAR/Udensenet_1/0.0001/0/',
                        metavar='FILE',
                        help="Training models of ICAR.")
    parser.add_argument('--model_path_ECAL',
                        default='/data/yilinzhi/Segmentation/VMS/checkpoints/semi_train_F/train/ECAL/Udensenet_TF_1/0.0001/0/',
                        metavar='FILE',
                        help="Training models of ECAL.")
    parser.add_argument('--model_path_ECAR',
                        default='/data/yilinzhi/Segmentation/VMS/checkpoints/semi_train_F/train/ECAR/Udensenet_TF_1/0.001/1/',
                        metavar='FILE',
                        help="Training models of ECAR.")

    parser.add_argument('--save_path', '-s',
                        default='/data/yilinzhi/Segmentation/VMS/checkpoints/semi_train_F/pred',
                        metavar='FILE',
                        help="Predicted result saving of four different positions.")
    ...
    # todo：注意不同情况要注释！
    parser.add_argument('case_name', metavar='INPUT',type=str,
...
def pred_post(args, case_name, model_name):
    ...
    # for CART in ['ECAL', 'ECAR']:
        ...
        if CART == 'ICAL':
            ...
            model_path = args.model_path_ECAL
            label_path = args.label_path_ECAL
        else:
            model_path = args.model_path_ECAR
            label_path = args.label_path_ECAR

        model.load_state_dict(torch.load(model_path + model_name))
        model.eval()

        create_dir(save_path + '/post_process/' + case_name + '_png_' + CART)  # 预测保存：病例+格式+位置
...
def png_contour_xml_post(case_name, pred_png_path, save_path):
    ...
    for CART in ['ICAL', 'ICAR']:
    # for CART in ['ECAL', 'ECAR']:
        pred_png_path_1 = pred_png_path
        pred_png_path_1 = pred_png_path_1 + CART
        # print(pred_png_path_1)
        pred_png_list = os.listdir(pred_png_path_1)  # 找到所有预测图片文件名
        pred_png_list.sort()
        ...
        for file in pred_png_list:  # 处理每个图片，file：文件名
            blob = io.imread(pred_png_path_1 + '/' + file)  # 读取文件
...
# python predict_semi_submit.py 0_P002_U

def pred_raw(args, model_name):
    """预测一个测试集：原始数据集"""

    case_name = str(args.case_name)  # 获取案例名字
    save_path = args.save_path  # 预测结果保存位置：png图片

    test_case_path = args.test_raw_case_path + case_name  # 获取测试集位置
    img_files = glob.glob(test_case_path + '/' + '*.dcm')  # 读取测试集原始dcm文档
    img_files.sort()

    dcm_img = read_dicom(test_case_path)  # 读取一个病例所有的dcm文件，并转成一个numpy文件
    for CART in ['ICAL', 'ICAR','ECAL','ECAR']: # 预测图片四个不同位置的血管mask
        # 初始化不同模型
        # model = UNet(1, 1)
        # model = udensedown(1,1)
        model = udensenet(1, 1)
        ...
        create_dir(save_path + '/raw/png/' + case_name + '_' + CART)  # 预测图片保存位置：病例+位置
...
def png_contour_qvs_raw(args):
    ...
    # TODO：生成QVS文件
    case_name = args.case_name  # 获取案例名字
    ...
    for CART in ['ICAL', 'ICAR','ECAL','ECAR']:  # 一个病例保存四个不同未知的QVS(xml）文件
        pred_png_path = args.save_path + '/raw/png/' + case_name + '_' + CART + '/'  # 预测图片保存位置
        ...
        create_dir(args.save_path + '/raw/QVS/' + case_name + '/CASCADE/CASCADE-' + CART + '/')  # QVS保存路径：一个病例四个不同位置
        ...
        tree = ET.ElementTree(QVAS_Series)
        tree.write(save_path, encoding='UTF-8', xml_declaration=True)
        print('%s %s QVS file finished.'%(name,CART))
    ...
    #
    #     pred_png_path = args.save_path + '/raw/png/' + case_name + '_' + CART + '/'  # 预测图片位置文件夹
    #     pred_png_list = os.listdir(pred_png_path)  # 找到所有预测图片文件名
    #     pred_png_list.sort(key=lambda name: int(name.split('_')[0]))
    #
    #     for file in pred_png_list:  # 处理每个图片，file：文件名
    #         blob = io.imread(pred_png_path + '/' + file)  # 读取文件
    #         index = int(file.split('/')[-1].split('_')[0])  # 得到对应的索引号
    #         # print("index:", index)
    #
    #         contours, hier = cv2.findContours(blob, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_NONE)  # 获取轮廓点坐标
    #
    ...
    for CART in ['ICAL', 'ICAR', 'ECAR', 'ECAL']:  # 一个病例保存四个不同未知的QVS(xml）文件
        pred_png_path = args.save_path + '/raw/png/' + case_name + '_' + CART + '/'
        pred_png_list = os.listdir(pred_png_path)  # 找到所有预测图片文件名
        pred_png_list.sort(key=lambda name: int(name.split('_')[0]))

        QVAS_Project_name = prefix + name + suffix_qvj  # QVJ文件名
        # create_dir(args.save_path + '/raw/QVS/' + case_name + '/CASCADE/CASCADE-' + CART + '/')  # QVS保存路径文件夹
        ...
        SeriesName = prefix + name + suffix_qvj_1
        BifurcationImageIndex.set('SeriesName', SeriesName)

        tree = ET.ElementTree(QVAS_Project)
        tree.write(save_path, encoding='UTF-8', xml_declaration=True)
        print('%s %s QVJ file finished.' % (name, CART))



if __name__ == '__main__':
    test_patience_internal = ["P206", "P432", "P576", "P891"]  # ICAL/ICAR的测试集
    test_patience_external = ["P429"]  # ECAL/ECAR的测试集
    args = get_args()

    # for case_name in test_patience_internal:
    ...
    # for case_name in test_patience_external:
    #     model_name = 'Udensenet_80.pkl'
    #     pred_post(args, case_name, model_name)
    #
    # for case_name in test_patience_external:
