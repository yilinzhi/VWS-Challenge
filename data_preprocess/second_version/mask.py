...
def save_mask_for_training(input_dir, save_path):
    """存储血管标注mask：标注的正样本可以确定，负样本展示有点问题。"""

    dir_create(save_path + '/' + 'ICAL')
    dir_create(save_path + '/' + 'ICAR')
    dir_create(save_path + '/' + 'ECAL')
    dir_create(save_path + '/' + 'ECAR')
        ...
        for arti in ['ICAL', 'ICAR', 'ECAL', 'ECAR']:
            ...
            if len(avail_slices):
                ...
                for index in range(len(avail_slices)):
                    ...
                    np.save(save_path + '/' + arti + '/' + 'positive_' + pi + '_' + arti + '_' + str(
                        avail_slices[index]) + '_.npy',
                            mask_save)

            if len(not_avail_slices):
                print("case", pi, "art", arti, "not_avail_slices", not_avail_slices)
                for index in not_avail_slices:
                    ...
                    np.save(save_path + '/' + arti + '/negative_' + pi + '_' + arti + '_' + str(
                        index) + '_.npy',
                            neg_mask_save)
...
def main(args):
    ...
    save_path = args.circle_mask_save_sep_train

    save_mask_for_training(input_dir, save_path)
