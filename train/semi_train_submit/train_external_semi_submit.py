...
# @Time    : 2021/06/02 18:07
# @Author  : Yi
# @FileName: train_external_semi_submit.py
...
def split_val(img_path, mask_path, folds, k, batch_size1, batch_size2):
    ...
    """
    # TODO：训练集+验证集：external left/right样本不一样！
    # file_names = ['P887', 'P732', 'P556', 'P723', 'P530']
    file_names =['P887','P891','P438','P125','P530']
...
def train_net(
    ...
    shutil.copyfile("train_external_semi_submit.py", check_path + '/train_external_semi_submit.py')  # 保存代码，即保存相关参数。
    ...
    for fold in range(folds):
        ...
        # net.load_state_dict(
        #     torch.load(
        #         "/data/yilinzhi/Segmentation/VMS/checkpoints/semi_train_F/train/ICAL/Udensenet_1/0.0001/0/Udensenet_80.pkl"),
        #     False)

        net.load_state_dict(
            torch.load(
                "/data/yilinzhi/Segmentation/VMS/checkpoints/semi_train_F/train/ICAR/Udensenet_1/0.0001/0//Udensenet_80.pkl"),
            False)

        # TODO: 超参数调节！
        if fold > 2:  # hyper parameters adjusting
            break
        ...
        # scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'max', factor=0.1,threshold=0.01,
        ...
        sys.stdout = Logger(fold_check_path + '/Udensenet_EL_log.txt')
        ...
        # 训练集+验证集：有标签+无标签
        train_loader_label, train_loader_un, val_loader_label, val_loader_un = split_val(img_path, mask_path,
        ...
        for epoch in range(epochs):  # 每折，训练总的轮次，每个轮次都会训练所有的样本
            ...
            for i_batch, (data, un_data) in enumerate(zip(cycle(train_loader_label), train_loader_un)):
                ...
                #
                # loss_un = loss_un / len(true_idx)  # 计算无标签样本的平均值

                loss = loss_label + loss_un  # 总loss
            ...
            for val_i_batch, (val_data, un_val_data) in enumerate(zip(cycle(val_loader_label), val_loader_un)):
                with torch.no_grad():
                    ...
                    val_loss_un = torch.tensor(0.0, device=device, requires_grad=False)
                    ...
                    val_loss_un = float(val_loss_un) / len(val_true_idx)
                    ...
                    if val_batch_num_all_epochs % 100 == 0:
                        ...
                        for i in range(un_val_pred_masks.shape[0]):  # 无标签验证集
                            ...
                            writer.add_image("unLabel val image : fake mask: pred mask: diffence" + f"{i + 1}",
                                             torchvision.utils.make_grid(images_list, nrow=4, padding=10,
                                                                         normalize=True),
                                             global_step=val_batch_num_all_epochs)
            ...
            # scheduler.step(val_epoch_avg_dice)
...
if __name__ == '__main__':
    ...
    # img_path = args.image_ECAL_semi
    # mask_path = args.mask_ECAL_semi
    # split_val(data_path,img_path,mask_path,folds,4,batch_size1,batch_size2)

    # TODO: 同时修改自定义数据函数！
    # for lr in [0.01, 0.001, 0.0001]:
    #     print("##" * 20, "lr=", str(lr), "##" * 20)
    #
    #     check_path = args.check_ECAL_semi + '/Udensenet_TF_1/' + str(lr)
    #     data_path = args.datasets_path
    #     img_path = args.image_ECAL_semi
    #     mask_path = args.mask_ECAL_semi
    #     print(check_path)
    ...
    for lr in [0.001, 0.0001]:
        ...
        check_path = args.check_ECAR_semi + '/Udensenet_TF_1/' + str(lr)
        data_path = args.datasets_path
        img_path = args.image_ECAR_semi
        mask_path = args.mask_ECAR_semi
        print(check_path)
        train_net(device=device, check_path=check_path, data_path=data_path, img_path=img_path, mask_path=mask_path,
                  lr=lr, folds=folds, epochs=epochs, batch_size1=batch_size1, batch_size2=batch_size2)
