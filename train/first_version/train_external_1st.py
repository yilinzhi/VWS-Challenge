...
# @Time    : 2021/04/20 23:04
# @Author  : Yi
# @FileName: train_external_1st.py
...
def train_net(
    ...
    shutil.copyfile("train_separate.py", check_path + '/train_separate.py')
    ...
    for fold in range(kfolds):
        ...
        # TODO: loading pretrained model(parameters instead of whole models)
        # net = UNet(1, 1)
        net = udensenet(1, 1)  # model structures
        net = nn.DataParallel(net, device_ids=[0, 1])
        net.to(device=device)
        # net.load_state_dict(
        #     torch.load("/data/yilinzhi/Segmentation/VMS/checkpoints/ICAR/Udensenet_DA_V1/0.0001/0/Udensenet_90.pkl"))
        net.load_state_dict(
            torch.load("/data/yilinzhi/Segmentation/VMS/checkpoints/ICAR/Udensenet_NDA/0.001/0/Udensenet_90.pkl"))
        ...
        sys.stdout = Logger(fold_check_path + '/Udensenet_el_nda_tl_log.txt')
...
if __name__ == '__main__':
    """Hyper parameters:
        ...
        Saving train_separate.py in each fold.
    ...
    #     # check_path = args.train_check_sep_ICAL + '/Unet_DA_1/' + str(lr)
    #     check_path = args.train_check_sep_ICAL + '/Unet_NDA/' + str(lr)
    #
    ...
    # ECAL: data_aug(angle/shear:10) + Unet: terrible
    # ECAL: data_aug(angle/shear:10) + Udensenet: testing
    # for lr in [0.1, 0.01, 0.001, 0.0001]:
    #     print("#" * 20, "lr=", str(lr), "#" * 20)
    #     image_path = args.train_image_sep_ECAL
    #     mask_path = args.train_mask_sep_ECAL
    #     check_path = args.train_check_sep_ECAL + '/Udensenet_DA_V1_Transfer/' + str(lr)
    #     # check_path = args.train_check_sep_ECAL + '/Unet_NDA/' + str(lr)
    #     train_net(device=device, image_path=image_path, mask_path=mask_path, check_path=check_path, args=args, lr=lr,
    #               kfolds=kfolds, epochs=epochs, batch_size=batch_size)

    # TODO: testing
    # ECAR：data_aug(angle/shear:10) + Udensenet + ICAR(state_dict): testing
    for lr in [0.1,0.01,0.001,0.0001,0.00001]:
        print("#" * 20, "lr=", str(lr), "#" * 20)
        image_path = args.train_image_sep_ECAR
        mask_path = args.train_mask_sep_ECAR
        # check_path = args.train_check_sep_ECAR + '/Udensenet_DA_V1_Transfer/' + str(lr)
        check_path = args.train_check_sep_ECAR + '/Udensenet_NDA_Transfer/' + str(lr)
