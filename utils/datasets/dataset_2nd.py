...
# @Time    : 2021/05/09 22:29
# @Author  : Yi
# @FileName: dataset_2nd.py
...
def data_augmentation(is_traning, image, mask):
    ...
    if is_traning == True:
        ...
        angle = float(random.randint(-10, 10))  # 旋转角度
        p3 = random.random()
        if p3 > 0.5:
...
class MyDataset_train(Dataset):
    ...
    def __init__(self, img_train_list,mask_train_list, state, args):
        super(MyDataset_train, self).__init__()

        self.img_train_list = img_train_list  # image训练集列表
        self.mask_train_list = mask_train_list  # mask训练集列表
        self.state = state
        self.args = args
    ...
    def __getitem__(self, idx):
        if self.state == "train":
            ...
            image_positive_train = np.load(
                os.path.join(self.args.train_image_sep_ICAL, self.img_train_list[idx]))
            mask_positive_train = np.load(
                os.path.join(self.args.train_mask_sep_ICAL, self.mask_train_list[idx]))
...
class MyDataset_validation(Dataset):
    ...
    def __init__(self, img_val_list,mask_val_list, state, args):
    ...
    def __getitem__(self, idx):
        if self.state == "validation":
            image_positive_validation = np.load(
                os.path.join(self.args.train_image_sep_ICAL, self.img_val_list[idx]))
            mask_positive_validation = np.load(
                os.path.join(self.args.train_mask_sep_ICAL, self.mask_val_list[idx]))
...
if __name__ == "__main__":
    image_path = "/data/yilinzhi/Segmentation/VMS/datasets/train_data/image_sep_position_train/ECAL/positive_P887_ECAL_322_.npy"
    mask_path = "/data/yilinzhi/Segmentation/VMS/datasets/train_label/circle_mask_sep_train/ECAL/positive_P887_ECAL_322_.npy"
