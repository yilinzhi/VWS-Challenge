...
# @Time    : 2021/05/29 09:08
# @Author  : Yi
# @FileName: dataset_semi_try.py
...
def data_augmentation(is_training, image, mask):
    #  image/mask：PIL or tensor, here is PIL
    if is_training == True:  # 进行数据增强
    ...
    elif is_training == False:  # 不进行数据增强
        image = F.to_tensor(np.array(image))
        mask = F.to_tensor(np.array(mask))
        return image, mask
    else:
...
class MyDataset_train(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        split_name = self.img_train_list[idx].split('/')[-1].split('_')
        label = split_name[-2]
        index = split_name[-3]
        position=split_name[-4]
        case=split_name[-5]
...
class MyDataset_val(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        split_name = self.img_val_list[idx].split('/')[-1].split('_')
        label = split_name[-2]
        index = split_name[-3]
        position=split_name[-4]
        case=split_name[-5]
        ...
        is_training = False  # 是否进行数据增强
        image_aug, mask_aug = data_augmentation(is_training, image_val, mask_val)
...
if __name__ == "__main__":
    image_path = "/data/yilinzhi/Segmentation/VMS/datasets/train_data/image_semi/ECAL/posLabel_P887_ECAL_322_.npy"
    mask_path = "/data/yilinzhi/Segmentation/VMS/datasets/train_label/mask_semi/ECAL/posLabel_P887_ECAL_322_.npy"
